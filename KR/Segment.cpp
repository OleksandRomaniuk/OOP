	...
	_a(x1, y1), _b(x2, y2), myId(++freeID)
{
	++_call;
#ifndef NDEBUG
	std::cout << myId << ": a Segment was created: " << *this;
#endif
};
...
Segment::Segment(const Segment& s) : _a(s._a), _b(s._b), myId(++freeID)
{
	++_call;
#ifndef NDEBUG
	std::cout << myId << ": a Segment was copied: " << *this;
#endif
}
Segment::~Segment()
{
	++_call;
#ifndef NDEBUG
	std::cout << myId << ": a Segment was deleted: " << *this;
#endif
}
...
int Segment::epilog()
{
	std::cout << "N calls " << _call << " Total amount of created Segments " << freeID << std::endl;
	return freeID;
}

std::ostream& operator<<(std::ostream& os, const Segment& u)
{
	os << u.getID() << '[' << u.start() << ',' << u.end() << ']' << std::endl;
	return os;
}